...
Рисунок 7 — Результат запроса на пользователей форума
...
Запрос на выдачу количества веток для каждого форума.

Листинг 12 — Запрос на количество веток в каждом форуме
```sql
...
В ходе данной лабораторной работы мы закрепили знания в области составления 
запросов и анализа их плана, а также в области их оптимизации.


-------------------------------------------------------------------

Лабораторная работа №3 «Нагрузочное тестирование и безопасность БД»
===================================================================

# Цель работы:

Продемонстрировать знания в области безопасности СУБД, используя SQL инъекции. 
Провести нагрузочное тестирование вашей БД, 
с помощью средств СУБД или используя дополнительное ПО.


# Нагрузочное тестирование

Скрипт на Go реализует нагрузочное тестирование реализованного API,
покрывающий весь его функционал (Рисунок 1).
...
Максимальный RPS составил **1583 запросов в секунду**

![Рисунок 1 — Вывод нагрузочного тестирования](/docs/perf_go.jpg)

Рисунок 1 — Вывод нагрузочного тестирования

![Рисунок 2 — Системный монитор](/docs/perf.jpg)

Рисунок 2 — Системный монитор


## Безопасность БД

Цель атаки в форме SQL-инъекции – сделать запросы к базе данных, 
манипулируя с параметрами приложения. 
Атака может быть возможна из-за некорректной обработки входных данных, 
используемых в SQL-запросах.

Напрмер, у нас есть запрос на получение сообщений внутри темы по её названию:
```
/api/threads/<slug>/posts/
```

Возможна SQL-инъекция в случае, если формирование запроса к будет происходить 
путем конкантенации строк:
```js
slug = request.get('slug')
query = "SELECT P.author, P.message " + 
    "FROM post P JOIN thread T on P.thread_id = T.id " + 
    "WHERE T.id = '" + slug + "' ORDER BY P.created;";
```

Вместо `<slug>` передадим строку вида:
```
'UNION SELECT U.nickname, U.password FROM person U --
```

В результяте мы получим список пользователей с их паролями.

Чтобы такого не происходило необходимо параметры передавать через 
параметризованные запросы:
```cpp
MapSqlParameterSource params = new MapSqlParameterSource();
params.addValue("slug", request.get('slug'));
NamedParameterJdbcTemplate.queryForObject(
        "SELECT P.author, P.message " + 
            "FROM post P JOIN thread T on P.thread_id = T.id " + 
            "WHERE T.id = :slug ORDER BY P.created;",
        params, 
        POST_MAPPER
...
В ходе данной лабораторной работы мы закрепили знания в области 
создания SQL-инъекций, а также методов защиты от них. 
Также мы провели нагрузочное тестирование, используя JMeter, 
для различного числа пользователей.
